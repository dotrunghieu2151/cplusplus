...
template <typename T, std::size_t N> constexpr void bubble_sort(T (&arr)[N]) {
...
template <typename T> constexpr void bubble_sort(std::vector<T>& arr) {
  int hasSwapped{false};
...
template <typename Iterator,
          std::invocable<Iterator, Iterator> F = bool(*)(Iterator, Iterator)>
constexpr void bubble_sort(
    const Iterator start, const Iterator end,
    F compareFn = [](Iterator a, Iterator b) -> bool { return *a > *b; }) {
  int hasSwapped{false};
...
template <typename T, std::size_t N>
constexpr int binary_search(const std::array<T, N>& arr, T target, int min,
                            int max) {
  if (min > max)
    return -1;
  int midPoint{std::midpoint(min, max)};
...
template <typename T>
constexpr int binary_search(const std::vector<T>& arr, T target, int min,
                            int max) {
  if (min > max)
    return -1;
  int midPoint{std::midpoint(min, max)};
